...
const ResumeUpload: React.FC = () => {
  ...
  const isFileValid = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleUpload = async () => {
    ...
    if(jobDescription && jobDescription.length) {
    ...
    try {
      ...
      navigate(`/interviews/${result.data.sessionId}`)
  ...
  return (
    <div className="p-5 font-sans flex flex-col w-[40%]">
        <label className="text-lg font-semibold text-gray-800 mb-2">Upload your Resume</label>
        <input type="file" id="resume" name="resume" accept=".pdf,.docx,.doc,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/msword" onChange={isFileValid} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100
        ...
        <label htmlFor="jobDescription" className='text-lg font-semibold text-gray-800 mb-2'>Job Description</label>
        <textarea name="jobDescription" id="jobDescription" onChange={(e) => setjobDescription(e.target.value)}/>
